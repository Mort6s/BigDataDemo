...
//        subQueries.addMetric("metric_fund").addTag("product", "HQD").addAggregator(zimsum).addDownsample("1s-" + zimsum);
        subQueries.addMetric("metric1").addTag("tag1", "test").addAggregator(zimsum).addDownsample("1ms-" +
                zimsum);
        long now = new Date().getTime() / 1000;
        builder.getQuery().setMsResolution(true);   // 返回的点的精度设置为毫秒级
